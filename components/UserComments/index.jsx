...
function UserComments({ isAdvancedEnabled }) {
  ...
  const renderCommentWithMentions = (comment) => {
    // Convert @mentions in the comment to clickable links
    return comment.split(/(@\[[^\]]+\]\([^)]+\))/).map((part, index) => {
      if (part.startsWith("@[")) {
        const match = part.match(/@\[(.+?)\]\((.+?)\)/); // Extract mention text and user ID
        return match ? (
        ...
        {comments.length ? (
          comments.map((comment) => (
            ...
            >
              ...
              <ListItemText
                primary={renderCommentWithMentions(comment.comment)} // Render mentions as links
                secondary={`Posted by ${comment.user.first_name} ${comment.user.last_name}`}
              />
            </ListItem>
          ))
